...
function postnavigation(items, index) {
    ...
    if (items[index - 1] !== undefined && index - 1 !== -1) {
        ...
        output.push(<Link to={"/blog-details/" + parseInt(id)} key={id} className='sigma_single-pagination-item pagination-prev'><span>Previous Post</span><h6>{item.title.slice(0, 20)}</h6> </Link>);
    }
    if (items[index + 1] !== undefined && index <= items.length - 1) {
        // Show next button 
        item = items[index + 1];
        id = item.id;
        output.push(<Link to={"/blog-details/" + parseInt(id)} key={id} className='sigma_single-pagination-item pagination-next'><span>Next Post</span><h6>{item.title.slice(0, 20)}</h6></Link>);
    }
...
export { getBlog, getCategories, getTags, getAuthor, getFilteredPosts, postnavigation, getRecentPost };